...
ph_rotation_motion::ph_rotation_motion(std::string ip, uint16_t port)
{
    std::cout << "creating linear axis client" << std::endl;
    _rotation_motion_struct.ip = ip;
    _rotation_motion_struct.port = port;
...
wgm_feedbacks::enum_sub_sys_feedback ph_rotation_motion::set_center_position(double new_target)
{
    _rotation_motion_struct.phead_start_angle = new_target;
    return sub_success;
}
 ...
 * @param new_position
 */
wgm_feedbacks::enum_sub_sys_feedback ph_rotation_motion::rotate_to(double new_position)
{
    if (new_position < 0) 
    {
...
wgm_feedbacks::enum_sub_sys_feedback ph_rotation_motion::connect()
{
    ...
    std::cout << "axis server ip:  " << _rotation_motion_struct.ip << std::endl;
    _client = new sockpp::tcp_connector({ _rotation_motion_struct.ip, _rotation_motion_struct.port });
    // Implicitly creates an inet_address from {host,port}
    // and then tries the connection.
    if (!_client->is_connected()) {
        std::cerr << "Error connecting to axis server at "
            << sockpp::inet_address(_rotation_motion_struct.ip, _rotation_motion_struct.port)
...
wgm_feedbacks::enum_sub_sys_feedback ph_rotation_motion::disconnect()
{
    axisReady = false;
 ...
 *
 */
wgm_feedbacks::enum_sub_sys_feedback ph_rotation_motion::rotate_home()
{
 ...
 *
 */
double ph_rotation_motion::get_speed()
{
    double speed = 0;
...
wgm_feedbacks::enum_sub_sys_feedback ph_rotation_motion::set_speed(double_t new_val)
{
...
wgm_feedbacks::enum_sub_sys_feedback ph_rotation_motion::rotate_up_to(double_t new_pos)
{
...
wgm_feedbacks::enum_sub_sys_feedback ph_rotation_motion::rotate_down_to(double_t new_pos)
{
 ...
 * @param steps
 */
wgm_feedbacks::enum_sub_sys_feedback ph_rotation_motion::rotate_up_by(double_t steps)
{
 ...
 * @param steps
 */
wgm_feedbacks::enum_sub_sys_feedback ph_rotation_motion::rotate_down_by(double_t steps)
{
...
wgm_feedbacks::enum_sub_sys_feedback ph_rotation_motion::rotate_center()
{
    auto command = axis_cmds.find("rotate");
    if (command != axis_cmds.end()) {
        std::cout << "sending command: " << command->second << '\n';
        ...
        std::string args = std::to_string(-_rotation_motion_struct.phead_start_angle);
        auto cmd = (command->second) + args;
        if (reply == "ok") return sub_success;
        return sub_error;
    }
...
wgm_feedbacks::enum_sub_sys_feedback ph_rotation_motion::unlock()
{
